...
function installPeerDeps() {
  ...
  getPackageConfig(rootPath, function(config) {
    ...
    console.log('installYarn', installYarn);
    ...
    if (installYarn) {
      ...
      console.error(
        'Did not find a viable package manager to install dependencies with.'
      );
    }
  });
}
...
function installDone(tool, result) {
  ...
  console.log(
    'Installed peerDependencies as devDependencies via ' + tool + '.'
  );
...
function getPeerInstallOptions(config) {
  ...
  if (
    typeof config.peerInstallOptions === 'object' &&
    !Array.isArray(config.peerInstallOptions)
  ) {
